...
// Temp-storage multer instance â€” accepts .zip or .db files
const upload = multer({
    dest: os.tmpdir(),
    limits: { fileSize: 2 * 1024 * 1024 * 1024 }, // 2 GB for zips with gcode
});

// GET /api/database/export â€” download a zip containing the DB + all gcode/uploads
...
// POST /api/database/import â€” restore from a .zip (DB + uploads) or legacy .db file
...
router.post('/import', upload.single('database'), (req, res) => {
    ...
    try {
        ...
        if (isSqlite) {
            // Legacy .db import â€” just replace the database
            closeDb();
        ...
        // ZIP import â€” extract DB + uploads
