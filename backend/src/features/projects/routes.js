...
// POST /api/projects (Hybrid â€” supports legacy single-template and new multi-instance)
router.post('/', (req, res) => {
...
// POST /api/projects/:id/instances â€” add a template instance to an existing project
...
// DELETE /api/projects/:id/instances/:instanceId â€” remove a template instance from a project
...
// PATCH /api/projects/:id/swap-option â€” swap a choice category's selected alternative
...
// POST /api/projects/:id/plates — add loose files to an existing project
router.post('/:id/plates', (req, res) => {
    ...
    try {
        ...
        const maxOrder = db.prepare('SELECT MAX(sort_order) as m FROM project_plates WHERE project_id = ?').get(project.id);
