...
// GET /api/themes â€” list all themes from themes.txt with install status
router.get('/', (req, res) => {
...
// POST /api/themes â€” add one or more URLs (newline-separated), git clone each
router.post('/', async (req, res) => {
    ...
    for (const rawUrl of newUrls) {
        ...
        try {
            ...
            if (!info?.cssPath) {
                results.push({ url: rawUrl, repoName, action, error: 'custom.css not found â€” is this a Mainsail theme repo?' });
            } else {
...
// DELETE /api/themes/:name â€” remove theme from list and delete its directory
