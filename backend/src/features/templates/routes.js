...
router.put('/:id', (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // Remap project_plates.template_plate_id from old â†’ new IDs
            ...
            // Build composite key â†’ [new_ids] for matching
            const newByKey = {};
            for (const np of newTemplatePlates) {
                const key = `${np.filename}|${np.cat_name || ''}|${np.opt_name || ''}`;
                (newByKey[key] = newByKey[key] || []).push(np.id);
            }
            // Build old_id â†’ new_id by matching on same composite key, consuming in order
            const oldByKey = {};
