...
const { clearSpoolCache } = require('../../services/spoolCache');
...
// live MQTT sniff. tray_info_idx = setting_id with 'S' removed (GFS_L99 â†’ GFL99).
...
function getNearestBambuColor(hexStr) {
    ...
    // Handles 3-char, 6-char, and 8-char (RRGGBBAA) inputs â€” alpha is always ignored.
    const hexToRgb = (h) => {
...
// GET /api/spoolman/spools â€” list all spools from Spoolman
...
// GET /api/spoolman/spool/:id/qr â€” proxy Spoolman QR code image (SVG/PNG)
...
// GET /api/spoolman/spool/:id â€” single spool details
...
// POST /api/spoolman/set-active â€” set active spool on a printer
    ...
    try {
        if (printer.firmware_type === 'bambu') {
            ...
            if (spoolId) {
                ...
                // Strip trailing digits/variants so e.g. "TPU95A" â†’ "TPU", "PA12" â†’ "PA12" (kept if in map)
            ...
            // Moonraker printer â€” original behavior
...
// GET /api/spoolman/ams-slots/:printerId â€” current spoolâ†’slot mapping for a Bambu printer
...
// â”€â”€ Multi-tool slot management (Klipper/Moonraker multi-toolhead) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// GET /api/spoolman/tool-slots/:printerId â€” { 0: spoolId|null, 1: spoolId|null, â€¦ }
...
// POST /api/spoolman/tool-slots/:printerId â€” { toolIndex, spoolId } assign or clear a slot
router.post('/tool-slots/:printerId', async (req, res) => {
    ...
    try {
        ...
        // Send gcode â€” printer may be offline, slot assignment is still saved
        try {
...
// POST /api/spoolman/activate-tool â€” { printerId, toolIndex }
...
// GET /api/spoolman/bambu-warnings â€” spools with untracked Bambu usage
...
// DELETE /api/spoolman/bambu-warnings/:spoolId â€” dismiss a Bambu usage warning
...
// PUT /api/spoolman/spool/:id/use â€” consume filament by length (mm) or weight (g)
...
// PUT /api/spoolman/spool/:id/measure â€” set filament amount by current measured gross weight (g)
// body: { weight: number }  â€” total weight of spool on a scale
...
// GET /api/spoolman/vendors â€” list all vendors
...
// POST /api/spoolman/vendors â€” create a vendor
...
// GET /api/spoolman/filaments â€” list all filaments
...
// POST /api/spoolman/filaments â€” create a filament
...
// POST /api/spoolman/spools â€” create a spool
...
router.post('/spools', async (req, res) => {
    ...
    try {
        ...
        } catch { /* non-critical â€” spool was still created */ }
        res.json(created);
    } catch (err) {
        res.status(502).json({ error: err.message });
    }
...
// PATCH /api/spoolman/spools/:id â€” partial update (proxy to Spoolman)
...
// DELETE /api/spoolman/spools/:id â€” delete a spool
...
// GET /api/spoolman/fields/:entity â€” get custom field definitions (entity: filament | vendor | spool)
...
// POST /api/spoolman/fields/:entity â€” create a custom field definition
...
// PATCH /api/spoolman/filaments/:id â€” update a filament
...
// DELETE /api/spoolman/filaments/:id â€” delete a filament
...
// POST /api/spoolman/filaments/:id/swatch â€” generate swatch STL, save to data/swatches/, return path
...
// PATCH /api/spoolman/vendors/:id â€” update a vendor
...
// DELETE /api/spoolman/vendors/:id â€” delete a vendor
...
// â”€â”€ Inventory (Marathon-local targets only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// GET /api/spoolman/inventory â€” list all tracked filament targets
...
// PUT /api/spoolman/inventory/:filamentId â€” upsert target/min for a filament
...
// DELETE /api/spoolman/inventory/:filamentId â€” stop tracking a filament
...
// GET /api/spoolman/storage-location â€” return the configured storage location name
...
// PUT /api/spoolman/storage-location â€” update the storage location name
...
// GET /api/spoolman/settings â€” fetch Spoolman global settings (currency, etc.)
...
/** Known material modifiers â€” parsed from material string for filtering. */
...
/** Parse a material string into { base, modifier }. e.g. "PLA Silk" â†’ { base: "PLA", modifier: "Silk" } */
function parseMaterial(material) {
    if (!material) return { base: '', modifier: '' };
    // Check extra field first â€” handled caller-side
...
// GET /api/spoolman/export/preview â€” return filaments with vendor/material info for selection UI
...
// GET /api/spoolman/export â€” download all Spoolman data as a JSON backup
...
// POST /api/spoolman/export â€” partial export filtered by filament/vendor IDs
...
// POST /api/spoolman/import/validate â€” check if backup fields exist on target
...
// POST /api/spoolman/import â€” restore from an exported JSON backup
// Recreates vendors â†’ filaments â†’ spools in order, remapping IDs.
router.post('/import', async (req, res) => {
    ...
    try {
        ...
        // Vendors
        push(`Importing ${data.vendors?.length || 0} vendorsâ€¦`);
        for (const v of (data.vendors || [])) {
            ...
            try {
                ...
                // Already exists â€” remap to the existing vendor's ID so filaments link correctly
        ...
        // Filaments
        if (currencyRate) push(`Converting prices with rate Ã—${currencyRate}`);
        push(`Importing ${data.filaments?.length || 0} filamentsâ€¦`);
        ...
        // Spools
        push(`Importing ${data.spools?.length || 0} spoolsâ€¦`);
        let spoolsOk = 0;
...
// POST /api/spoolman/import/from-db â€” import from a Spoolman SQLite .db or .zip backup
...
router.post('/import/from-db', dbUpload.single('database'), async (req, res) => {
    ...
    try {
        ...
        if (isSqlite) {
            ...
            if (jsonEntry) {
                push(`Detected Marathon JSON backup (${jsonEntry.entryName})â€¦`);
        ...
        // â”€â”€ Read source data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // â”€â”€ Push to Spoolman API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // Vendor ID mapping: old DB id â†’ new Spoolman id
        const vendorIdMap = {};
        push(`Importing ${vendors.length} vendorsâ€¦`);
        ...
        // Filament ID mapping: old DB id â†’ new Spoolman id
        const filamentIdMap = {};
        push(`Importing ${filaments.length} filamentsâ€¦`);
        ...
        // Spools
        push(`Importing ${spools.length} spoolsâ€¦`);
        let spoolsOk = 0;
        for (const s of spools) {
            const body = {};
...
// GET /api/spoolman/exchange-rate?from=USD&to=EUR â€” fetch live exchange rate
...
// â”€â”€ Service status (bundled containers / external URLs) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// GET /api/spoolman/services/status â€” deploy mode + Spoolman reachability
...
// GET /api/spoolman/test â€” test connection to Spoolman
...
// â”€â”€ Teamster (load cell scale) proxy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/** GET /api/spoolman/teamster/weight â€” fetch live weight from the Teamster ESP device */
...
/** POST /api/spoolman/teamster/tare â€” zero the scale */
...
/** POST /api/spoolman/teamster/calibrate â€” calibrate with a known weight */
...
/** GET /api/spoolman/teamster/test â€” check connectivity to the Teamster device */
